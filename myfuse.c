...
int myfuse_getattr(const char * path, struct stat * result) {
    char realname[strlen(path)]; // delete the "\" before path
    ...
    if (strcmp(path, "/") == 0) {
        ...
        int size;
        size = file_size((char *)realname, helper);
        if(size >= 0) {
            result -> st_size = size;
        }
        else {
...
int myfuse_rename(const char *oldname, const char *newname) {
    char realname[strlen(oldname)]; // delete the "\" before path
    memcpy(realname, oldname + 1, strlen(oldname));
    char newrealname[strlen(newname)]; // delete the "\" before path
...
int myfuse_create(const char *path, mode_t mode, struct fuse_file_info *ffi) {
    char realname[strlen(path)]; // delete the "\" before path
    memcpy(realname, path + 1, strlen(path));
    int result = create_file((char *)realname, (size_t)0, helper);
    if(result == 1) {
        return -EEXIST;
    }
    if(result == 2) {
